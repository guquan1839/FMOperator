"""FMO-MABSI: MLP-parameterised Bilinear Sum Interaction with per-head
attentive pair weighting.

This is the combination of FMO_MBSI (per-head MLP interaction factors) and
FMO_ABSI (input-dependent weighting of the six field pairs), with the
difference that every head scores the pairs with its *own* learned vector, so
each head can specialise on a different pair of fields.
...
Pair weighting
--------------
    logit_h(i, j) = <w_h, phi_h(i, j)>,        phi_h in R^r
    alpha_h       = softmax_{pairs} (logit_h)      ("per_head", default)
    S_h           = P * sum_{i<j} alpha_h(i, j) * phi_h(i, j)

With ``pair_pool="shared"`` a single softmax is shared by all heads (the
FMO-ABSI behaviour) and with ``pair_pool="sum"`` the weights are uniform, i.e.
exactly FMO_MBSI.  ``w_h`` is initialised to zero, so training starts from the
uniform sum and the attention has to earn its non-uniformity.
"""
...
__all__ = ["HistoryEncoder", "make_head_projection", "FMO_MABSI", "FIELD_NAMES", "FIELD_PAIRS"]
...
def make_head_projection(
    ...
    ``proj_hidden <= 0`` returns a bias-free linear projection; a positive
    value returns a two-layer MLP with hidden activation ``mid_act`` and
    optional output activation ``out_act`` (``"identity"`` by default).
    """
...
class FMO_MABSI(nn.Module):
    """FMO with per-head MLP interaction factors and per-head pair attention.

    Parameters
    ----------
    pair_pool:
        ``"per_head"`` (default) -> one softmax per head;
        ``"shared"`` -> one softmax shared by all heads;
        ``"sum"`` -> uniform weights, i.e. exactly FMO_MBSI (ablation only).
    Other parameters are identical to FMO_MBSI.
    """
    ...
    def __init__(
        ...
        # FMO_MBSI, so the two models are exactly equal at initialisation.
        self.gate = (
...
if __name__ == "__main__":  # tiny self-check, no data required
    ...
    print(
        ...
        f"pool={model.pair_pool} proj_hidden={model.proj_hidden}"
    )
